...
export function MobileNav({ content, locale, onToggleLocale }: MobileNavProps) {
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-[11px] px-3 py-2 rounded-xl border border-black/10 text-black/60 hover:text-black hover:border-black/20 hover:bg-black/[0.03] transition-all duration-200 tracking-wide font-sans"
            >
              {locale === "fa" ? "EN" : "فا"}
            </button>
          ...
          <div className="rounded-2xl border border-black/[0.06] px-2 py-2 flex flex-col" style={NAV_STYLE}>
            {content.nav.map((l) => (
              <a
                ...
                href={l.href}
                onClick={close}
                className="px-4 py-3 text-sm text-black/60 hover:text-black hover:bg-black/[0.03] rounded-xl transition-colors tracking-wide font-sans"
              >
                {l.label}
              </a>
            ))}
